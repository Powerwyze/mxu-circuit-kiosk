...
/* ─────────────────────  HOME / SPONSORS  ────────────────────── */
(() => {
  const list = (window.MXU_SPONSORS || []);
  const left  = $("#sponsorRailLeft");
  const right = $("#sponsorRailRight");
  if (!left || !right || !list.length) return;
  ...
  function tile(s, idx){
    ...
    btn.type = "button";
    btn.className = `sponsor-tile sponsor-tile--${s.accent || "red"}`;
    btn.dataset.sponsorIdx = String(idx);
    btn.setAttribute("aria-label", `${s.name} — ${s.tag}`);
    btn.innerHTML = `
      <span class="sponsor-tile__mono" aria-hidden="true">${initials(s.name)}</span>
      <span class="sponsor-tile__name">${s.name}</span>
      <span class="sponsor-tile__tag">${s.tag}</span>
    `;
    return btn;
  }

  // Split into left/right rails (alternating) so both sides show variety
  list.forEach((s, i) => (i % 2 === 0 ? left : right).appendChild(tile(s, i)));

  // Sponsor modal
  const sm        = $("#sponsorModal");
  const smLogo    = $("#sponsorModalLogo");
  const smTitle   = $("#sponsorModalTitle");
  const smTag     = $("#sponsorModalTag");
  const smLink    = $("#sponsorModalLink");
  const smHint    = $("#sponsorModalHint");

  function openSponsor(s){
    smLogo.textContent  = initials(s.name);
    smLogo.className    = `smodal__logo smodal__logo--${s.accent || "red"}`;
    smTitle.textContent = s.name;
    smTag.textContent   = s.tag || "";
    if (s.url) {
      smLink.style.display = "";
      smLink.disabled = false;
      smLink.onclick = () => {
        closeSponsor();
        openIframe(s.url, s.name);
      };
      smHint.textContent = s.url.replace(/^https?:\/\//, "");
    } else {
      smLink.style.display = "none";
      smHint.textContent = "Coming soon";
    }
  ...
  // Open on tile click
  document.addEventListener("click", (e) => {
    const t = e.target.closest("[data-sponsor-idx]");
    if (!t) return;
    const idx = parseInt(t.dataset.sponsorIdx, 10);
    if (Number.isFinite(idx) && list[idx]) openSponsor(list[idx]);
  });
  // Close handlers
  document.addEventListener("click", (e) => {
    if (e.target.closest("[data-smodal-close]")) closeSponsor();
  });
  document.addEventListener("keydown", (e) => {
    if (e.key === "Escape" && sm.classList.contains("is-open")) closeSponsor();
  });
})();
